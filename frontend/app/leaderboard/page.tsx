...
export default function LeaderboardPage() {
  ...
  return (
    <div className="min-h-screen pb-24">
      {/* Header */}
      <div className="bg-cream-100 border-b border-[rgba(184,134,11,0.12)] px-4 pt-12 pb-2">
      ...
      <div className="max-w-lg mx-auto px-4 pt-4 space-y-4">
        {/* Leaderboard list */}
        <div className="bg-ivory-card rounded-xl shadow-card border border-[rgba(184,134,11,0.12)] p-4">
          {entriesLoading ? (
            <div className="space-y-3">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="flex items-center gap-3 py-2 animate-pulse">
                  <div className="w-8 h-8 bg-gold-light rounded-full flex-shrink-0" />
                  <div className="flex-1">
                    <div className="h-3 w-28 bg-gold-light rounded" />
                  </div>
                  <div className="h-3 w-16 bg-gold-light rounded" />
                </div>
              ))}
            </div>
          ) : !entries || entries.length === 0 ? (
            <div className="text-center py-10">
              <div className="w-12 h-12 rounded-full bg-gold-pale flex items-center justify-center mx-auto mb-3">
